...
class VQModel(nn.Module):
    def __init__(self, config: ModelArgs):
        super().__init__()
        self.config = config
        self.Cvae, self.vocab_size = config.codebook_embed_dim, config.codebook_size
    ...
    def img_to_reconstructed_multi_scale_img(self, x, v_patch_nums=None):
        if v_patch_nums is None:
            ...
            all_quant, diff = self.multi_scale_encode(x, v_patch_nums)
    ...
    @torch.no_grad()
    def img_to_idxBl(self, x, v_patch_nums):
        h = self.encoder(x)
        all_indices = []
        for hw in v_patch_nums:
            _h = F.interpolate(h.clone(), size=(hw, hw), mode='area')
            _h = self.quant_conv(_h)
            _, _, info = self.quantize(_h)
            all_indices.append(info[2].reshape(x.shape[0], -1))
        return all_indices
...
def VQ_8(**kwargs):
    return VQModel(ModelArgs(encoder_ch_mult=[1, 2,], decoder_ch_mult=[1, 2,],codebook_size=1024, **kwargs))
